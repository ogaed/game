...
import * as Animatable from 'react-native-animatable';r
...
const questionsData = {
  ...
  export default function QuizScreen() {
    ...
    const [usedHints, setUsedHints] = useState([false, false]); // Track used hints for each level
    ...
    const handleAnswer = () => {
      ...
      if (userAnswer.toLowerCase() === question.answer.toLowerCase()) {
        ...
        if (currentQuestion < level.questions.length - 1) {
          ...
          setUsedHints([false, false]); // Reset used hints for the new level
        }
      } else {
        setTrials(trials + 1);
        if (trials >= 2) {
    ...
    const goToNextLevel = () => {
      if (currentLevel < questionsData.levels.length - 1 && currentQuestion === questionsData.levels[currentLevel].questions.length - 1) {
        setCurrentLevel(currentLevel + 1);
        setCurrentQuestion(0);
        setUsedHints([false, false]); // Reset used hints for the new level
      }
    ...
    const goToPreviousLevel = () => {
      if (currentLevel > 0) {
        setCurrentLevel(currentLevel - 1);
        setCurrentQuestion(0);
        setUsedHints([false, false]); // Reset used hints for the previous level
      }
    };
    ...
    const handleNext = () => {
        ...
        } else if (currentLevel < questionsData.levels.length - 1) {
            setCurrentLevel(currentLevel + 1);
    ...
    const handlePrevious = () => {
        if (currentQuestionIndex > 0) {
            ...
            setCurrentLevel(currentLevel - 1);
            setCurrentQuestionIndex(questionsData.levels[currentLevel - 1].questions.length - 1);
        }
    };
    ...
    //     setAnswers({
    //         ...answers,
    //         [`${currentLevel}-${currentQuestionIndex}`]: true
    //     });
    //     handleNext();
    // };
